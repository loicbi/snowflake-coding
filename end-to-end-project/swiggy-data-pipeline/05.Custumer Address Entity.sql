...
CUSTOMER ADDRESS ENTITY :
    - STAGE LAYER ->  DEMO_DB.ALF_STAGE_SCH.customeraddress 
    - CLEAN LAYER ->  DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_ADDRESS
    - CONSUMPTION LAYER  ->  DEMO_DB.ALF_CONSUMPTION_SCH.CUSTOMER_ADDRESS_DIM
 */
...
create or replace table DEMO_DB.ALF_STAGE_SCH.customeraddress (
...
create or replace stream DEMO_DB.ALF_STAGE_SCH.customeraddress_stm 
on table DEMO_DB.ALF_STAGE_SCH.customeraddress
append_only = true
comment = 'This is the append-only stream object on customer address table that only gets delta data';


select * from DEMO_DB.ALF_STAGE_SCH.customeraddress_stm;
copy into DEMO_DB.ALF_STAGE_SCH.customeraddress (addressid, customerid, flatno, houseno, floor, building, 
...
from (
    ...
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/initial/customer-address t
)
...
-- 2nd layer
CREATE OR REPLACE TABLE DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_ADDRESS (
...
create or replace stream DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_ADDRESS_STM
on table DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_ADDRESS
comment = 'This is the stream object on customer address entity to track insert, update, and delete changes';


MERGE INTO DEMO_DB.ALF_CLEAN_SCH.customer_address AS clean
USING (
    SELECT 
        ...
        _stg_file_md5,
        _copy_data_ts
    FROM DEMO_DB.ALF_STAGE_SCH.customeraddress_stm 
) AS stage
...
SELECT COUNT(*) FROM DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_ADDRESS_STM;




CREATE OR REPLACE TABLE DEMO_DB.ALF_CONSUMPTION_SCH.CUSTOMER_ADDRESS_DIM (
...
-- select * from DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_ADDRESS_STM;
MERGE INTO 
    DEMO_DB.ALF_CONSUMPTION_SCH.CUSTOMER_ADDRESS_DIM AS target
USING 
    DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_ADDRESS_STM AS source
ON 
...
--
select * from DEMO_DB.ALF_STAGE_SCH.customeraddressbook;
select * from DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_ADDRESS_BOOK;
select * from DEMO_DB.ALF_CONSUMPTION_SCH.CUSTOMER_ADDRESS_BOOK_DIM;


list @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/customer-address;
copy into DEMO_DB.ALF_STAGE_SCH.customeraddress (addressid, customerid, flatno, houseno, floor, building, 
                               landmark, locality,city,pincode, state, coordinates, primaryflag, addresstype, 
                               createddate, modifieddate, 
                               _stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/customer-address/ t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.csv_file_format')
on_error = abort_statement;