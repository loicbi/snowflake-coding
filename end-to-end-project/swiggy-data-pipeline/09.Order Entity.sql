...
CUSTOMER ADDRESS ENTITY :
    - STAGE LAYER ->  DEMO_DB.ALF_STAGE_SCH.orders 
    - CLEAN LAYER ->  DEMO_DB.ALF_CLEAN_SCH.ORDERS
...
create or replace table DEMO_DB.ALF_STAGE_SCH.orders (
...
create or replace stream DEMO_DB.ALF_STAGE_SCH.orders_stm 
on table DEMO_DB.ALF_STAGE_SCH.orders
append_only = true
comment = 'This is the append-only stream object on orders entity that only gets delta data';

list  @DEMO_DB.ALF_STAGE_SCH.csv_stg/initial/orders/orders-initial.csv;


copy into DEMO_DB.ALF_STAGE_SCH.orders (orderid, customerid, restaurantid, orderdate, totalamount, 
...
from (
    ...
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/initial/orders/orders-initial.csv t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.csv_file_format')
on_error = abort_statement;


CREATE OR REPLACE TABLE DEMO_DB.ALF_CLEAN_SCH.ORDERS (
...
create or replace stream DEMO_DB.ALF_CLEAN_SCH.ORDERS_stm 
on table DEMO_DB.ALF_CLEAN_SCH.ORDERS
comment = 'This is the stream object on ORDERS table table to track insert, update, and delete changes';


MERGE INTO DEMO_DB.ALF_CLEAN_SCH.ORDERS AS target
USING DEMO_DB.ALF_STAGE_SCH.ORDERS_STM AS source
...
-- part-2
list @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/orders/;
copy into DEMO_DB.ALF_STAGE_SCH.orders (orderid, customerid, restaurantid, orderdate, totalamount, 
                  status, paymentmethod, createddate, modifieddate,
                  _stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/orders/day-02-orders.csv t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.csv_file_format')
on_error = abort_statement;

SELECT * FROM DEMO_DB.ALF_CLEAN_SCH.ORDERS;