...
CUSTOMER ADDRESS ENTITY :
    - STAGE LAYER ->  DEMO_DB.ALF_STAGE_SCH.deliveryagent 
    - CLEAN LAYER ->  DEMO_DB.ALF_CLEAN_SCH.delivery_agent
...
create or replace table DEMO_DB.ALF_STAGE_SCH.deliveryagent (
...
create or replace stream DEMO_DB.ALF_STAGE_SCH.deliveryagent_stm 
on table DEMO_DB.ALF_STAGE_SCH.deliveryagent
append_only = true
comment = 'This is the append-only stream object on delivery agent table that only gets delta data';


copy into DEMO_DB.ALF_STAGE_SCH.deliveryagent (deliveryagentid, name, phone, vehicletype, locationid, 
...
from (
    ...
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/initial/delivery-agent t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.csv_file_format')
on_error = abort_statement;

select count(*) from DEMO_DB.ALF_STAGE_SCH.deliveryagent;

select * from DEMO_DB.ALF_STAGE_SCH.deliveryagent_stm;


CREATE OR REPLACE TABLE DEMO_DB.ALF_CLEAN_SCH.delivery_agent (
...
create or replace stream DEMO_DB.ALF_CLEAN_SCH.delivery_agent_stm 
on table DEMO_DB.ALF_CLEAN_SCH.delivery_agent
comment = 'This is the stream object on delivery agent table table to track insert, update, and delete changes';


MERGE INTO DEMO_DB.ALF_CLEAN_SCH.delivery_agent AS target
USING DEMO_DB.ALF_STAGE_SCH.deliveryagent_stm AS source
...
select * from DEMO_DB.ALF_CLEAN_SCH.delivery_agent_stm ;


CREATE OR REPLACE TABLE DEMO_DB.ALF_CONSUMPTION_SCH.delivery_agent_dim (
...
MERGE INTO DEMO_DB.ALF_CONSUMPTION_SCH.delivery_agent_dim AS target
USING DEMO_DB.ALF_CLEAN_SCH.delivery_agent_stm AS source
ON 
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/delivery-agent/day-02-delivery-agent.csv t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.csv_file_format')
on_error = abort_statement;
