...
CUSTOMER ADDRESS ENTITY :
    - STAGE LAYER ->  DEMO_DB.ALF_STAGE_SCH.menu 
    - CLEAN LAYER ->  DEMO_DB.ALF_CLEAN_SCH.menu
    - CONSUMPTION LAYER  ->  DEMO_DB.ALF_CONSUMPTION_SCH.menu_dim
 */
...
create or replace table DEMO_DB.ALF_STAGE_SCH.menu (
...
create or replace stream DEMO_DB.ALF_STAGE_SCH.menu_stm 
on table DEMO_DB.ALF_STAGE_SCH.menu
append_only = true
comment = 'This is the append-only stream object on menu entity that only gets delta data';


list @DEMO_DB.ALF_STAGE_SCH.csv_stg/initial/menu;

copy into DEMO_DB.ALF_STAGE_SCH.menu (menuid, restaurantid, itemname, description, price, category, 
...
create or replace stream DEMO_DB.ALF_CLEAN_SCH.menu_stm 
on table DEMO_DB.ALF_CLEAN_SCH.menu
comment = 'This is the stream object on menu table table to track insert, update, and delete changes';


MERGE INTO DEMO_DB.ALF_CLEAN_SCH.menu AS target
USING (
    SELECT 
        ...
        _stg_file_md5,
        _copy_data_ts
    FROM DEMO_DB.ALF_STAGE_SCH.menu
) AS source
...
CREATE OR REPLACE TABLE DEMO_DB.ALF_CONSUMPTION_SCH.menu_dim (
...
MERGE INTO 
    DEMO_DB.ALF_CONSUMPTION_SCH.MENU_DIM AS target
USING 
    DEMO_DB.ALF_CLEAN_SCH.MENU_STM AS source
ON 
...
-- Part -2
list @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/menu;
copy into DEMO_DB.ALF_STAGE_SCH.menu (menuid, restaurantid, itemname, description, price, category, 
                availability, itemtype, createddate, modifieddate,
                _stg_file_name, _stg_file_load_ts, _stg_file_md5, _copy_data_ts)
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/menu/ t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.csv_file_format')
on_error = abort_statement;