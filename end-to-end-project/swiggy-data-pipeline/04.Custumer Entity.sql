...
create or replace table DEMO_DB.ALF_STAGE_SCH.customer (
    customerid text,                    -- primary key as text
    name text,                          -- name as text
    mobile text WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'PII'),                        -- mobile number as text
    email text WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'EMAIL'),                         -- email as text
    loginbyusing text,                  -- login method as text
    gender text WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'PII'),                        -- gender as text
    dob text WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'PII'),                           -- date of birth as text
...
create or replace stream DEMO_DB.ALF_STAGE_SCH.customer_stm 
on table DEMO_DB.ALF_STAGE_SCH.customer
append_only = true
...
copy into  DEMO_DB.ALF_STAGE_SCH.customer (customerid, name, mobile, email, loginbyusing, gender, dob, anniversary, 
...
from (
    ...
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/initial/customer/customers-initial.csv t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.csv_file_format')
on_error = abort_statement;

select * from DEMO_DB.ALF_STAGE_SCH.customer limit 10;
select count(*) from DEMO_DB.ALF_STAGE_SCH.customer; -- 99899
select count(*) from DEMO_DB.ALF_STAGE_SCH.customer_stm; 
select * from DEMO_DB.ALF_STAGE_SCH.customer_stm; ;


-- Part-2 Clean Layer
-- 

CREATE OR REPLACE TABLE DEMO_DB.ALF_CLEAN_SCH.CUSTOMER (
    ...
    MOBILE STRING(15)  WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'PII'),                                           -- Mobile number, accommodating international format
    EMAIL STRING(100) WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'EMAIL'),                                           -- Email
    LOGIN_BY_USING STRING(50),                                   -- Method of login (e.g., Social, Google, etc.)
    GENDER STRING(10)  WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'PII'),                                           -- Gender
    DOB DATE WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'PII'),                                                    -- Date of birth in DATE format
...
-- Stream object to capture the changes. 
create or replace stream DEMO_DB.ALF_CLEAN_SCH.customer_stm 
on table DEMO_DB.ALF_CLEAN_SCH.customer
comment = 'This is the stream object on customer entity to track insert, update, and delete changes';


-- insert into DEMO_DB.ALF_CLEAN_SCH.customer (
--     customer_id,
--     name,
--     mobile,
--     email,
...
--     _stg_file_md5,
--     _copy_data_ts
-- from DEMO_DB.ALF_STAGE_SCH.customer;


MERGE INTO DEMO_DB.ALF_CLEAN_SCH.CUSTOMER AS target
USING (
    SELECT 
        ...
        _STG_FILE_MD5,
        _COPY_DATA_TS
    FROM DEMO_DB.ALF_STAGE_SCH.CUSTOMER_STM
) AS source
...
select count(*) from DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_STM; 

select * from DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_STM; 


-- create dim table 
CREATE OR REPLACE TABLE DEMO_DB.ALF_CONSUMPTION_SCH.CUSTOMER_DIM (
    CUSTOMER_HK NUMBER PRIMARY KEY,               -- Surrogate key for the customer
    CUSTOMER_ID STRING NOT NULL,                                 -- Natural key for the customer
    NAME STRING(100) NOT NULL,                                   -- Customer name
    MOBILE STRING(15) WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'PII'),                                           -- Mobile number
    EMAIL STRING(100) WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'EMAIL'),                                           -- Email
    LOGIN_BY_USING STRING(50),                                   -- Method of login
    GENDER STRING(10) WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'PII'),                                           -- Gender
    DOB DATE WITH TAG (DEMO_DB.ALF_COMMON.PII_POLICY_TAG = 'PII'),                                                    -- Date of birth
...
MERGE INTO 
    DEMO_DB.ALF_CONSUMPTION_SCH.CUSTOMER_DIM AS target
USING 
    DEMO_DB.ALF_CLEAN_SCH.CUSTOMER_STM AS source
ON 
...
SELECT * FROM  DEMO_DB.ALF_CONSUMPTION_SCH.CUSTOMER_DIM;

// ----------------------------------------------------------
// ----------------------------------------------------------
-- delta processing check

list @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/customer/;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/customer/day-01-insert-customer.csv t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.csv_file_format')
on_error = abort_statement;
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/customer/day-02-insert-update.csv t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.csv_file_format')
on_error = abort_statement;