...
on table DEMO_DB.ALF_STAGE_SCH.restaurant
append_only = true
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @DEMO_DB.ALF_STAGE_SCH.csv_stg/initial/restaurant/restaurant-delhi+NCR.csv t
)
...
USING (
    SELECT 
        ...
        _stg_file_md5
    FROM 
        DEMO_DB.ALF_STAGE_SCH.restaurant_stm
) AS source
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp() as _copy_data_ts
     from @DEMO_DB.ALF_STAGE_SCH.csv_stg/delta/restaurant/day-02-upsert-restaurant-delhi+NCR.csv t
)
file_format = (format_name = 'DEMO_DB.ALF_STAGE_SCH.CSV_FILE_FORMAT')
on_error = abort_statement;
